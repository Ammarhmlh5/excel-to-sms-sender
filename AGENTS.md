...
| `src/components/SettingsDialog.tsx` | API key + password management |
| `supabase/functions/send-sms/index.ts` | Edge Function: validates, rate-limits, sends via hloov.com, tracks campaigns |
| `supabase/functions/register-device/index.ts` | Edge Function: mobile app device registration |
| `supabase/functions/verify-jwks/index.ts` | Edge Function: JWT verification + account linking |
...
- All times in UTC
- Edge Functions use service role key for rate-limit writes via `adminClient`
