...
serve(async (req) => {
  ...
  try {
    ...
    // Simple auth for admin endpoints: require header x-admin-key matching env
    const adminKey = Deno.env.get('ADMIN_API_KEY') || serviceRoleKey;
    ...
    if (req.method === 'POST') {
      // Support POST-based invocation from Supabase client with action field
    ...
    if (req.method === 'POST') {
      ...
      // Create new delivery_attempt to requeue
      ...
      // Mark campaign_message as pending so retry worker or send flow can pick it
