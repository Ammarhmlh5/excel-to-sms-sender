...
import { CampaignStatusBadge, MessageStatusBadge } from '@/shared/components/StatusBadges';
import { formatDate } from '@/shared/lib/formatDate';
import { toggleAdminRole } from '@/admin/lib/adminActions';
    ...
    (async () => {
      ...
      try {
        ...
        if (!error && result) {
          ...
          toast.error(error?.message || 'فشل في جلب بيانات المستخدم');
        }
      } catch {
                  ...
                  {data.apiKeys.length === 0 ? (
                    ...
                    data.apiKeys.map((k) => (
                      <TableRow key={k.id}>
                        ...
                        <TableCell dir="ltr" className="text-xs font-mono">{k.api_key.substring(0, 4)}****{k.api_key.slice(-4)}</TableCell>
                        <TableCell>
